  ...
  getStation = (AStation) => {
    ...
    this.state.radioStations.forEach(function (AItem) {
      if (AItem.id == AStation) {
        if (AItem.logosource) {
          result = (
            <Link href={'/radio-station/' + AStation} native>
              <img class={style['station-logo']} alt={AItem.name} title={AItem.name} src={AItem.logosource} />
            </Link>
          );
        } else {
          result = (
        ...
        <div class={'page-container'}>
          ...
          <main class={'content ' + style.playlist}>
            ...
            <div class={'btn-container btn-container--right'}>
              <button onClick={this.playPlaylist} class={'btn btn--play'}>
                Listen now
              </button>
            </div>
            {canEdit ? (
              <div class={'btn-container btn-container--left'}>
                <Link href={href.replace('/playlist/', '/playlist-edit/')} native class={'margin--right btn btn--edit ' + style['btn--playlist']}>
                  Edit
                </Link>
                <button onClick={this.deletePlaylist} class={'btn btn--cancel'}>
                  Delete
                </button>
              </div>
            ) : null}
            <ul class={style['playlists']}>
              {items.map((scheduleItem) => (
                <li
                  class={
                    style['playlists__item'] +
                    ' ' +
                    style['hours'] +
                    ' ' +
                    style['hours--' + this.getHours(scheduleItem.startHour, scheduleItem.endHour)] +
                    ' ' +
                    style[this.isActive(scheduleItem.startHour, scheduleItem.endHour)]
                  }
                >
                  <time>
                    {this.getTimeCaption(scheduleItem.startHour)}
                    <button onClick={this.playPlaylist} class={'btn ' + style['btn--on-air']}>
                      On air
                    </button>
                  </time>
