    ...
    return (
      <div class={'page-container'}>
        ...
        <main class={'content ' + style.settings}>
          <h1 class={'main-title'}>
            {docTitle}
            <small class={'main-subtitle'}>🎚️ What does this button do?</small>
          </h1>
          <h2 class={style['content-title']}>Theme</h2>
          <p class={style.selecttheme}>
            <DropDown
              initialValue={settings && settings.theme ? settings.theme : 'default'}
              optionList={this.state.themeOptions}
              valueChanged={this.themeOptionChanged.bind(this)}
            />
          </p>
          ...
          <button class={'btn btn--secondary'} onClick={this.resetLocalPreferences.bind(this)}>
            Reset now
          </button>
          ...
          <div class={style.experimental}>
            ...
            {!settings || !settings.experimental || !settings.experimental.chromecast ? (
              <p>
                Pssst! I'm working on Chromecast support. Want to try? OK, might be buggy, but 🤞 and{' '}
                <button onClick={this.enableChromecastSupport.bind(this)} class={'btn btn--small btn--secondary'}>
                  Enable
                </button>{' '}
                Chromecast support
              </p>
              ...
              <p>
                ...
                . 🙂
                <br />
                Is the Chromecast functionality too buggy?{' '}
                <button onClick={this.disableChromecastSupport.bind(this)} class={'btn btn--small btn--secondary'}>
                  Disable
                </button>{' '}
                Chromecast support then... 🙁
              </p>
            )}
            {!settings || !settings.experimental || !settings.experimental.podcastindex ? (
              <p>
                ...
                as podcast search engine instead of Apple?{' '}
                <button onClick={this.enablePodcastindexSearch.bind(this)} class={'btn btn--small btn--secondary'}>
                  Enable
                </button>{' '}
                the Podcastindex.org search engine.
              </p>
            ) : (
              <p>
                Thanks for trying podcast search via{' '}
                <a href="https://podcastindex.org" target="_blank" rel="noopener">
                  Podcastindex.org
                </a>
                ...
                . 🙂
                <br />
                If you want to use Apple iTunes Search API again, then{' '}
                <button onClick={this.disablePodcastindexSearch.bind(this)} class={'btn btn--small btn--secondary'}>
                  Disable
                </button>{' '}
                Podcastindex.org support... 🙁
              </p>
            )}
          </div>
        </main>
