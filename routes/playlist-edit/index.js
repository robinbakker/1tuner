...
export default class PlaylistEdit extends Component {
  ...
  getCountryCode = (lang) => {
    if (!lang || !this.props.languageList || !this.props.languageList.length) {
      return null;
    }
    let Result = null;
    let languageList = this.props.languageList;
    for (let i = 0; i < languageList.length; i++) {
      if (languageList[i].id == lang) {
  ...
  getPlaylist = (playlistName, playlistHref) => {
    let result = null;
    playlistHref = playlistHref.substr(playlistHref.indexOf('/playlist-edit/'));
    playlistHref = playlistHref.replace('/playlist-edit/', '/playlist/');
    if (playlistHref.indexOf('?') >= 0) {
      let sUrlArr = playlistHref.split('?');
      let qp = sUrlArr[1];
      ...
      return (
        <div class={'page-container'}>
          ...
          <main class={'content ' + style.playlist}>
            ...
            {errorMessage && errorMessage != '' ? <p class={style['error-message']}>{errorMessage}</p> : null}
          </main>
        </div>
      );
    } else {
              ...
              <ul class={style['item-list']}>
                ...
                {items.map((scheduleItem) => (
                  <li class={style['item-list__item']}>
                    <EditScheduleItem
                      ...
                      stationOptionList={radioStations}
                      buttonClass="btn--remove"
                      buttonText="Remove"
                      rowChange={this.scheduleItemChanged}
                      buttonClick={this.removeScheduleItem}
                    ></EditScheduleItem>
                  </li>
                ))}
                <li class={style['item-list__item'] + ' ' + style['item-list__item--break']}>
                  <EditScheduleItem
                    ...
                    buttonClass="btn--add"
                    buttonText="Add"
                    buttonClick={this.addScheduleItem}
                    startHour={newStartHour}
                    endHour={newEndHour}
                    rowChange={this.addItemChanged}
                    stationOptionList={radioStations}
                  ></EditScheduleItem>
                </li>
              </ul>
              <div class={'btn-container btn-container--content-right'}>
                <Link href={href ? href : '/playlists'} native class={'btn btn--cancel margin--right'}>
                  Cancel
                </Link>
