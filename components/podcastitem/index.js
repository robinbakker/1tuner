		...
		if (podcastItem) {
			...
			return (
				<div class={'preset-list__item-content preset-list__item-content--podcast'}>
					<Link href={pcLink} title={podcastItem.name} class={'preset-list__button'  + (small ? ' preset-list__button--small' : '') + (podcastItem.logo ? '' : ' icon--podcast')} style={(podcastItem.logo ? '' : getColorString(podcastItem.collectionId))}>
						{podcastItem.logo ?
