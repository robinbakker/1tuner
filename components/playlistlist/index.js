    ...
    APlaylistSchedule.forEach(function (AItem) {
      if (stationArray.indexOf(AItem.station) == -1) {
        ...
        if (st) {
          if (st.logosource) {
            result.push(
              <li class={style['station-list__item']}>
                <img class={style['station-logo']} alt={st.name} title={st.name} src={st.logosource} />
              </li>
            );
          } else {
            result.push(
        ...
        <ul class={'preset-list' + (horizontal ? ' preset-list--horizontal' : ' preset-list--page')}>
          {items.map((playlistItem) => (
            <li class={'preset-list__item'}>
              {horizontal ? (
                <button
                  data-playlist={playlistItem.href}
                  onClick={this.setPlaylist.bind(this)}
                  title={playlistItem.name}
                  class={'preset-list__button' + (small ? ' preset-list__button--small' : '') + ' icon--playlist'}
                  style={'background-color:' + playlistItem.color}
                >
                ...
                <div class={'preset-list__item-content preset-list__item-content--playlist'}>
                  ...
                  {small ? null : (
                    <div class={'preset-list__link-content'}>
                      <Link href={playlistItem.href} class={'preset-list__link'}>
                        {playlistItem.name}
                      </Link>
                  ...
                  {small ? null : (
                    <button
                      ...
                      title={playlistItem.name}
                      title={'Play'}
                      class={'btn btn--secondary btn--play'}
                    ></button>
                  )}
                </div>
              )}
